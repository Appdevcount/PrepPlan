...
module "storage" {
  ...
  name = "tbtestgmsa${random_string.randomname.result}"
...
module "GoldenLinuxFuncApp" {
    ...
    app_service_plan_sku_name         = local.sku_name #When using a Elastic Plan,  "Always On" must be False.10.124
    ...
    #For Elastic Plans
    #elastic_instance_minimum          = 2 # Use only for Elastic Plan.
    #plan_maximum_elastic_worker_count = 4 #Only used with Elastic Plan = Maximum Burst. should be => app_worker_count.
    ...
    #Slots
    include_function_app_slot          = true  
    function_app_slot                  = {
                                           name                       = "staging"
                                           private_endpoint_subnet_id = local.pe_subnet_id 
                                           ip_restriction             = var.ip_restriction
    ...
    include_alerts                  = true
    alarm_funnel_resourcegroup_name = "AlarmFunnel-rg"
    alarm_funnel_action_group_name  = "AlarmFunnel-ag"
    alert_action_group_email        = "troy.braman@evicore.com"
}
