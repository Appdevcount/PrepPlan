...
resource "azurerm_linux_function_app" "function" {
  ...
  site_config {
    ...
    application_stack {
      ...
      use_dotnet_isolated_runtime = var.use_dotnet ? var.use_dotnet_isolated_runtime : null
      python_version              = var.use_python ? var.python_version : null
      java_version                = var.use_java ? var.java_version : null
      powershell_core_version     = var.use_powershell ? var.powershell_core_version : null
      node_version                = var.use_node ? var.node_version : null
  
      dynamic "docker" {
      for_each = var.use_docker ? ["use_docker"] : []
        content {
  ...
  lifecycle {
    ...
    precondition {
      condition     = var.include_alerts ? length(var.alert_action_group_email) > 0 && length(var.alarm_funnel_resourcegroup_name) > 0 && length(var.alarm_funnel_action_group_name) > 0 : true 
      error_message = "The following variables are required when including alerts: alert_action_group_email, alarm_funnel_action_group_subscription, resourcegroup_name_for_alarm_funnel_action_group, alarm_funnel_action_group_name."
    }
