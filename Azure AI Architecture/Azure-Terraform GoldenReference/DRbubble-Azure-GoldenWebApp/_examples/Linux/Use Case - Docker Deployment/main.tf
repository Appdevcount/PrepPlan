...
##NOTE: An ACR with private endpoints should already exist.
#How to create ACR with private endpoints and public network access disabled
/* resource "azurerm_container_registry" "acr" {
  name                          = "tbacrpoc"
  resource_group_name           = "tb-acr-poc"
...
/*   use_docker = true
  ...
  web_app_slot = {
    ...
    use_docker                 = true
    registry_url               = "https://tbacrpoc.azurecr.io"  #ACR URL
    image_name                 = "worldtripclient:ch2"          #Repository name and tag
...
resource "azurerm_role_assignment" "ra" {
  ...
  principal_id         = module.GoldenLinuxWebApp.app_identity
...
resource "azurerm_role_assignment" "ra_slot" {
  depends_on          = [module.GoldenLinuxWebApp]
  principal_id         = module.GoldenLinuxWebApp.app_slot_identity
  role_definition_name = "AcrPull"
  scope                = "/subscriptions/75dbc8c6-6364-4eb1-91c1-3400a4010fb4/resourceGroups/tb-acr-poc/providers/Microsoft.ContainerRegistry/registries/tbacrpoc"
  principal_type       = "ServicePrincipal"
}
...
#THIS OPTION IS NOW NATIVELY SUPPORTED AS PART OF THE GOLDEN MODULE
#resource "azapi_update_resource" "app_vnet_container_pull_routing" {
#  depends_on  = [module.GoldenLinuxWebApp]
#  resource_id = module.GoldenLinuxWebApp.app_id
...
#  depends_on  = [module.GoldenLinuxWebApp]
#  resource_id = module.GoldenLinuxWebApp.app_slot_id
