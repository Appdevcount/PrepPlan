...
module "GoldenWindowFuncApp" {
    ...
    name                              = "tb-gm-window-os-fa"
    resource_group_name               = var.resource_group_name
    service_plan_id                   = module.Azure-GoldenAppServicePlan.asp_id
    location                          = var.location
    #app_service_plan_name            = var.app_service_plan_name  #Add this and sku_name to have the module create ASP
...
/* resource "azurerm_role_assignment" "function_app_strg_role_assignment" {
  depends_on           = [module.storage, module.GoldenLinuxFuncApp]
