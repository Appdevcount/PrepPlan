...
## Mind Map — GraphQL vs REST at a Glance
...
```
    ...
    │          │          │                          │           │           │
┌───▼──┐  ┌───▼──┐  ┌───▼──┐               ┌───────▼──┐ ┌─────▼────┐ ┌───▼──────┐
│URLs  │  │Verbs │  │Fixed │               │ Single   │ │ Client-  │ │ Strongly │
│/res  │  │GET   │  │Shape │               │ Endpoint │ │ Driven   │ │ Typed    │
│/{id} │  │POST  │  │Resp  │               │ /graphql │ │ Shape    │ │ Schema   │
└───┬──┘  │PUT   │  └──────┘               └───────┬──┘ └─────┬────┘ └───┬──────┘
    │     │DEL   │                                  │          │          │
    │     └──────┘                                  └──────────┴──────────┘
    │                                                          │
    │   PROBLEMS                                    SOLVES ────┤
    │   ──────────────────────────────              ───────────┤
    │   • Over-fetching (too many fields)           • Fetch exactly what you need
    │   • Under-fetching (need N requests)          • 1 request → nested data
    │   • Versioning chaos (/v1, /v2, /v3)          • Evolve schema, no versioning
    │   • Rigid contract (server decides)           • Client owns the contract
    │                                                          │
    │                                               OPERATIONS ┤
    │                                               ───────────┤
    │                                               • Query      (≈ GET)
    ...
    │
    │   WHEN TO KEEP REST                           WHEN TO PICK GRAPHQL
    │   ──────────────────────────────              ────────────────────────────────
    │   • Simple CRUD, stable contract              • Multiple consumers (web/mobile)
    │   • File upload / binary data                 • Complex nested data graphs
    │   • HTTP caching is critical                  • Rapid UI iteration needed
    │   • Team unfamiliar with GQL                  • Over/under-fetch is a real pain
    │   • Public API (broad interop)                • BFF (Backend-for-Frontend) layer
    │
    └─── KEY DIFFERENCE ──────────────────────────────────────────────────────────
         REST: Server decides what you get.   GraphQL: Client decides what it needs.
```
